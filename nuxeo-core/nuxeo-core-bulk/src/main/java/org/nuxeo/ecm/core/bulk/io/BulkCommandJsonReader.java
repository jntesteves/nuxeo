...
public class BulkCommandJsonReader extends EntityJsonReader<BulkCommand> {
    ...
    @Override
    protected BulkCommand readEntity(JsonNode jn) throws IOException {
        ...
        if (jn.has(COMMAND_PARAMS)) {
            params = BulkParameters.paramsToMap(new ObjectMapper().writeValueAsString(jn.get(COMMAND_PARAMS)));
        }
